...
def split_batch_by_ratio(batch_size, ratio_list, test_env):
    batch_sizes = np.ones((len(ratio_list),), dtype=int) # batch_size>=1 for each domain
    remaining = batch_size - (len(ratio_list) - len(test_env))
    total_ratio = np.sum([r for d, r in enumerate(ratio_list) if d not in test_env])
    for domain, ratio in enumerate(ratio_list):
        if domain in test_env:
            continue
        num = round(remaining*ratio/total_ratio)
        batch_sizes[domain] += num
        remaining -= num
        total_ratio -= ratio
