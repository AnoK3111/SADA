...
@torch.no_grad()
def update_bn(iterator, model, n_steps, algorithm, device="cuda"):
    momenta = {}
    ...
    for i in range(n_steps):
        ...
        if algorithm.__class__.__name__=="StainContrast":
            x, _ = algorithm.transform.transform(x, torch.zeros((x.size(0),)))
