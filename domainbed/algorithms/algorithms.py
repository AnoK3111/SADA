...
from domainbed.losses import Local_Alignment_Loss, Supervised_Contrast_Loss, Domain_Proxy_Contrast_Loss
from domainbed.stain_augmentation import stain_augmentation
...
class AbstractDANN(Algorithm):
    ...
    def update(self, x, y, **kwargs):
        ...
        disc_labels = torch.cat(
            [
                torch.full((x.shape[0],), i, dtype=torch.int64, device=self.hparams["device"])
...
class IRM(ERM):
    ...
    def _irm_penalty(self, logits, y):
        scale = torch.tensor(1.0).to(self.hparams["device"]).requires_grad_()
...
class BoDA(ERM):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams, eps=1e-6):
    ...
    def init_env_labels(self, env_labels=None, device='cpu'):
        ...
        if env_labels is not None:
            ...
            self.n_samples_table = torch.tensor([
                count_samples_per_class(env_labels[env], self.num_classes) for env in sorted(env_labels)]).to(device)
    ...
    def update(self, x, y, **kwargs):
        self.update_feature_stats(kwargs['env_feats'])

        n_envs = len(x)
        all_y = torch.cat(y)
        all_envs = torch.cat([env * torch.ones_like(yi) for env, yi in enumerate(y)])
        features = [self.featurizer(xi) for xi in x]
        classifiers = [self.classifier(fi) for fi in features]
        targets = [yi for yi in y]
        ...
        if self.steps >= self.boda_start_step:
            pairwise_dist = -1 * self.pairwise_dist(self.train_centroids.to(features[0].device), torch.cat(features))
            # balanced distance
            ...
            log_prob = logits - torch.log((torch.exp(logits) * (1 - mask_same_d_c)).sum(0, keepdim=True) + self.eps) #eps for numerical stability
...
class Hybrid_SC(Algorithm):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(Hybrid_SC, self).__init__(input_shape, num_classes, num_domains, hparams)
        self.featurizer = networks.ResNet(input_shape, self.hparams)
        self.classifier = nn.Linear(self.featurizer.n_outputs, num_classes)
        self.network = nn.Sequential(
            self.featurizer, 
            self.classifier)
        self.projection = networks.projection(hparams)
        self.supervised_contrast_loss = Supervised_Contrast_Loss(hparams)
        self.optimizer = get_optimizer(
            hparams["optimizer"],
            self.parameters(),
            lr=self.hparams["lr"],
    ...
    def update(self, x, y, **kwargs):
        a = kwargs['step']/kwargs['max_step']
        
        all_x = torch.cat(x)
        all_y = torch.cat(y)
        all_features = self.featurizer(all_x)
        ce_loss = F.cross_entropy(self.classifier(all_features), all_y)

        all_projections = self.projection(all_features)
        sc_loss = self.supervised_contrast_loss(all_projections, all_y)
        
        loss = a * ce_loss + (1 - a) * sc_loss
...
class SADA(Algorithm):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(SADA, self).__init__(input_shape, num_classes, num_domains, hparams)
        self.featurizer = networks.ResNet(input_shape, self.hparams)
        self.classifier = nn.Linear(self.featurizer.n_outputs, num_classes)
        self.network = nn.Sequential(
            self.featurizer, 
        ...
        self.projection = networks.projection(hparams)
        self.mse = nn.MSELoss()
        self.transform = stain_augmentation(k=self.hparams["k"], device=hparams["device"])
        self.la_loss = Local_Alignment_Loss(hparams)
        # self.supervised_contrast_loss = Domain_Proxy_Contrast_Loss(hparams)
        self.supervised_contrast_loss = Supervised_Contrast_Loss(hparams)
        self.optimizer = get_optimizer(
            hparams["optimizer"],
            self.parameters(),
        ...
        self.register_buffer('all_feature_maps', torch.zeros(hparams["batch_size"] * hparams["k"] * 4, self.featurizer.n_outputs, 7, 7))
        def hook(model, in_fea, out_fea):
            self.all_feature_maps = out_fea
        
        self.featurizer.network.layer4.register_forward_hook(hook=hook)   
    ...
    def update(self, x, y, **kwargs):
        ...
        all_x = torch.cat(x)
        all_y = torch.cat(y)
        if kwargs['step'] >= self.hparams["self_supervised_step"]:
            all_x, all_y = self.transform.origin_samples(all_x, all_y)
            all_features = self.featurizer(all_x)
            loss = F.cross_entropy(self.classifier(all_features), all_y)
        else:
            all_x, all_y = self.transform.transform(all_x, all_y)
            batch_size = all_x.size(0)
            all_features = self.featurizer(all_x)
            la_loss = self.la_loss(self.all_feature_maps)

            # origin_features = all_features.reshape(batch_size // self.hparams["k"], self.hparams["k"], -1)[:,0]
            # origin_y = all_y.reshape(batch_size // self.hparams["k"], self.hparams["k"])[:,0]
            # all_projections = self.projection(origin_features)
            # sc_loss = self.supervised_contrast_loss(all_projections, origin_y)

            all_projections = self.projection(all_features)
            sc_loss = self.supervised_contrast_loss(all_projections, all_y)


            loss =  la_loss * self.hparams['la_weight'] + sc_loss
