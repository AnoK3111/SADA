...
def _hparams(algorithm, dataset, random_state):
    ...
    hparams["freeze_bn"] = (True, False)
    hparams["pretrained"] = (True, True)  # only for ResNet
    hparams['split_batch_by_ratio'] = (False, False)
    if 'BoDA' in algorithm:
        if dataset == 'DomainNet':
            hparams['boda_start_step'] = (3000, int(1000 * random_state.choice(range(2, 6))))
            hparams['feat_update_freq'] = (2000, int(10 * random_state.uniform(150, 300)))
        else:
            hparams['boda_start_step'] = (500, int(100 * random_state.choice(range(3, 7))))
            hparams['feat_update_freq'] = (120, int(random_state.uniform(100, 150)))
        hparams['nu'] = (0.5, 10**random_state.uniform(-0.5, 0))
        hparams['boda_weight'] = (.1, 10**random_state.uniform(-2, -0.5))
        hparams['temperature'] =  (1., 10**random_state.uniform(-1, 0.5))
        hparams['macro_weight'] = (1., 10**random_state.uniform(-1, 1))
        hparams['momentum'] = (.2, random_state.uniform(0, 0.4))
    ...
    if 'Hybrid_SC' in algorithm:
        hparams['projection_type'] = ("linear", None)
        hparams['projection_size'] = (128, int(2 ** random_state.uniform(3, 5)))
        hparams['temperature'] =  (0.1, 10**random_state.uniform(-1, 0.5))

    if 'SADA' in algorithm:
        hparams['projection_type'] = ("non-linear", None)
        hparams['projection_size'] = (128, int(2 ** random_state.uniform(3, 5)))
        hparams['local_projection_size'] = (128, int(2 ** random_state.uniform(3, 5)))
        hparams['la_weight'] =  (0.0, 10**random_state.uniform(-1, 0.5))
        hparams['k'] =  (3, 10**random_state.uniform(-1, 0.5))
        hparams['self_supervised_step'] =  (4000, 10**random_state.uniform(-1, 0.5))
        hparams['temperature'] =  (0.10, 10**random_state.uniform(-1, 0.5))
        hparams["batch_size"] = (32, int(2 ** random_state.uniform(3, 5)))
        hparams["domain_proxy_contrast_learning"] = (True, False)
        hparams['split_batch_by_ratio'] = (False, False)
