...
def main():
    ...
    parser.add_argument("--algorithm", type=str, default="SADA")
    parser.add_argument(
        "--trial_seed",
        type=int,
        default=0,
    ...
    table = PrettyTable(["(acc, f1_micro, f1_macro)"] + dataset.environments + ["Avg."])
    for key, row in results.items():
        row.append(np.mean(np.array(row),axis=0))
        row = [misc.get_res(acc) for acc in row]
